...
//02 Заполните объект city для тестов

test.skip('test city should contains 3 houses', () => {
  expect(city.houses.length).toBe(3); // Ожидаем что у сити есть хаусы и длина массива

  expect(city.houses[0].buildedAt).toBe(2012); // Ожидаем что домик построен в том году
...
// 01 Создайте тип govermentBuildings
//02 Заполните объект city для тестов
...
test.skip('test city should contains hospital and fire station', () => {
  ...
  expect(city.govermentBuildings[0].adress.street.title).toBe('Central Str');
